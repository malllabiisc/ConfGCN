...
class ConfGCN(object):
	...
	def dropout(self, inp, dropout, num_features_nonzero=0, is_sparse=False):
		if is_sparse: 	return self.sparse_dropout(inp, 1 - dropout, num_features_nonzero)
		else:		return tf.nn.dropout(inp, 1-dropout)

	def cal_conf(self, labels):
		return tf.matmul(labels, labels, transpose_b=True)


	def GCNLayer(self, gcn_in, adj_ind, adj_ind_mask, input_dim, output_dim, num_labels, act, dropout, num_features_nonzero, input_sparse=False, name='GCN'):
	...
	def evaluate(self, sess, split='valid'):
		...
		outs_val 	= sess.run([model.loss, model.accuracy], feed_dict=feed_dict) 	# Computer loss and accuracy
		return outs_val[0], outs_val[1]							# return loss, accuracy
